...
public class Example9 {
    public static void main(String[] args) throws Exception {
        ...
        env.addSource(new ClickSource())
                ...
                .process(new WindowResults())
                .print();

        env.execute();
    }

    public static class WindowResults extends ProcessWindowFunction<Event,String,String, TimeWindow> {
        ...
        public void process(String key, Context context, Iterable<Event> elements, Collector<String> out) throws Exception {
            ...
            long windowClose = context.window().getEnd();
            long count = elements.spliterator().getExactSizeIfKnown(); // 迭代器里面共多少条元素
            out.collect("用户：" + key + " 在窗口" +
                    "" + new Timestamp(windowStart) + "~" + new Timestamp(windowClose) + "" +
                    "中的pv次数是：" + count);
        }
    }
